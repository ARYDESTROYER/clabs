...
### Task: Understand Stored XSS and map the attack surface

### Objective
Understand what Stored XSS is, where it appears in this app, and how to start testing safely.

### Background
Cross-Site Scripting (XSS) happens when attacker-controlled input is rendered as executable HTML/JS.

Common types:
- Reflected XSS: payload is reflected immediately in a response.
- Stored XSS: payload is stored (DB) and executed when another user views it.
- DOM XSS: payload is handled client-side by insecure JS logic.

This lab focuses on **Stored XSS** in input forms on a web page.

### Tasks (explore the target application)
1. Open the app and browse these sections:
- `/`
- `/products`
- `/reviews`
- `/cart` (login required)
...
3. Open any product page and locate the review submission form.

4. Submit a harmless test review (plain text), then verify it is rendered back.

5. Return to `/` and check for reconnaissance completion (`FLAG1`).

### What to look for
- User input is stored and if they are displayed later.
- For example Username, Reviews etc are rendered in multiple places.
- This is a persistent execution surface, which is why Stored XSS is high impact.
