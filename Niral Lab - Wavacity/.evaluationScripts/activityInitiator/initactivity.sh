...
# Start offline Wavacity for audio spectrogram analysis in no-internet labs.
if [ -d "$WAVACITY_DIR" ]; then
	pkill -f "wavacity_server.py" 2>/dev/null || true
	nohup python3 "$EVAL_DIR/activityInitiator/wavacity_server.py" \
		--host 0.0.0.0 \
		--port "$WAVACITY_PORT" \
		--directory "$WAVACITY_DIR" \
		>/tmp/wavacity_server.log 2>&1 &
	echo "[+] Local Wavacity available at http://localhost:${WAVACITY_PORT}"
else
	echo "[!] Wavacity assets not found at $WAVACITY_DIR; use sox fallback."
fi
