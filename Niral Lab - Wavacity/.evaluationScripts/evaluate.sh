...
# Ensure autograder produced output before publishing to LMS path.
if [ ! -f "$TEMP_OUTPUT" ]; then
    echo "Error: Autograder did not produce $TEMP_OUTPUT" >&2
    exit 1
fi

# Copy the autograder output back to the mounted directory for the LMS.
if ! cp "$TEMP_OUTPUT" "$EVAL_DIR/evaluate.json"; then
    echo "Warning: Failed to publish evaluation result to $EVAL_DIR/evaluate.json" >&2
    echo "Hint: Mount may be read-only in LMS metadata layer." >&2
    echo "Warning: Autograder output is still available at $TEMP_OUTPUT" >&2
else
    echo "[+] Evaluation result published to $EVAL_DIR/evaluate.json"
fi
