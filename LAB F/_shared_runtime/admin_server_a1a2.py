#!/usr/bin/env python3
# LAB F internal admin server (Activities 1 and 2).
#
...
# this is the trust assumption DNS rebinding subverts.
...
# Activity 1 wants the student to confirm the admin is reachable from inside
# the container, so the welcome banner already contains FLAG1.
FLAG1 = "IITB{labf_a1_threat_model_mapped}"
# Activity 2 wants the student to drive the bot through a rebind to read this
# token; it's not handed out via the welcome banner.
FLAG2 = "IITB{labf_a2_dns_rebinding_works}"
...
def token():
    return jsonify({
        "session_token": FLAG2,
        "issued_to": "internal-staff",
        "note": "This token grants admin API access. Never expose to untrusted networks.",
    })
