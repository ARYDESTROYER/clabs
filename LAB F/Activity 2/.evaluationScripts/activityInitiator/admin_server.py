#!/usr/bin/env python3
# LAB F Activity 2 internal admin server.
#
# Bound to 127.0.0.1:8080. NOT exposed to the LMS port mapping.
# Trusts loopback callers -- the realistic vulnerability rebinding exploits.
...
def welcome():
    return jsonify({
        "msg": "Corporate admin panel v2.",
        "note": "Sensitive endpoints live under /admin/*. Trusted to loopback only.",
    })
...
def token():
    return jsonify({
        ...
        "warning": "Never expose this endpoint to untrusted networks.",
    })
